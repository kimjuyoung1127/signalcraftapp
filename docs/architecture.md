...
graph TB
    ...
    subgraph "State Management"
        D --> AB[useAuthStore]
        D --> AC[useDeviceStore]
        D --> AD[useDiagnosisLogic (Hook State)] // Updated
    end
...
sequenceDiagram
    ...
    RN->>HK: handleUpload()
    HK->>SV: uploadAudio(uri)
    SV->>FP: POST /upload (Multipart)
    FP-->>SV: Task ID 반환
...
### 🔧 백엔드 구조 개선 (Phase 3.1)
- **Feature-based Backend**: `app/features/audio_analysis`로 오디오 분석 관련 로직(모델, 스키마, 라우터, 서비스)을 응집.
- **비동기 처리**: Celery를 이용한 Non-blocking 분석 요청 처리.

---

**문서 버전**: 2.5 (AR Diagnosis System 반영)
**작성일**: 2025-11-23
**마지막 수정**: 2025-11-26 (Phase C+ 완료 반영)
